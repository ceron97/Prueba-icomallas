<template>
	<div class="container">
		...
		<div v-else>
			<MyForm :id="userId" :formType="formType" />
...
<script>
import MyForm from "@/components/FormUsuarios.vue";
...
export default {
	...
	methods: {
		...
		// Elimina al usuario de los registros
		async deleteItem() {
			...
			try {
				await axios.delete(`http://localhost:8000/api/usuarios/borrar/${item.id}`, {
		...
		// Modifica el estado de el usuario
		async changeStatus() {
			...
			try {
				await axios.patch(
					`http://localhost:8000/api/usuarios/estado/${item.id}`,
					{ estado: !item.estado },
					{
						withCredentials: true,
					}
		...
		async refresh() {
			const idToast = this.$toast("Por favor espera...");
			try {
				const response = await axios.get("http://localhost:8000/api/usuarios/consultar", {
					withCredentials: true,
				});
